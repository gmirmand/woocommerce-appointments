		...
		init: function (cart) {
			...
			$cart
				...
				.on('woocommerce-product-addons-update', function () {
					...
					// Appointments compat.
					if (
						$('.wc-appointments-appointment-cost').length
					) {
						/*
						product_price = parseFloat( appointment_cost )
							: ( $( '.wc-appointments-appointment-cost' ).attr( 'data-raw-price' ) );
						*/
					}

					// Required checkboxes.
					$cart.find('.wc-pao-required-addon').each( function() {
						var $container = $( this );
						var $requiredCheckboxes = $container.find( ':checkbox[required]' );
					...
					$cart.find('.wc-pao-addon-field').each(function () {
						var $addon = $(this),
							...
							addon_duration = 0,
							addon_duration_raw = 0,
							duration_type = $addon.data('duration-type'),
						...
						if ($addon.is('.wc-pao-addon-custom-price')) {
							...
							addon_duration = $addon.data( 'duration' ) * $addon.val();
							addon_duration_raw =
								$addon.data('raw-duration') * $addon.val();
						} else if (
							$addon.is(
							...
							addon_duration = $addon.data('duration');
							addon_duration_raw = $addon.data('raw-duration');
						} else if (
							$addon.is(
							...
							if ($addon.val()) {
								...
								addon_cost_raw = $addon
									.find('option:selected')
									.data('raw-price');
								addon_duration = $addon
									.find('option:selected')
									.data('duration');
								addon_duration_raw = $addon
									.find('option:selected')
									.data('raw-duration');
							}
						} else {
						...
						switch (duration_type) {
							...
							case '':
								addon_data.duration =
									addon_duration_raw;
								addon_data.duration_raw =
									addon_duration_raw;
								break;
							case 'quantity_based':
								addon_data.duration_pu =
									addon_duration_raw;
								addon_data.duration_raw_pu =
									addon_duration_raw;
								addon_data.duration =
									addon_data.duration_pu * qty;
								addon_data.duration_raw =
									addon_data.duration_raw_pu * qty;
								break;
						}
						...
						if ('undefined' !== typeof value_label) {
							...
							addon_data.duration_type = duration_type;
			...
			var image_swatches = $('.wc-pao-addon-image-swatch');
			if (image_swatches.length) {
				image_swatches.tipTip({ delay: 200 });
			}
		},
	};
