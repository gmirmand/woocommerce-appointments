...
class WC_Product_Addons_Admin {
	...
	public function admin_menu() {
		$page = add_submenu_page( 'edit.php?post_type=product', __( 'Add-ons', 'woocommerce-appointments' ), __( 'Add-ons', 'woocommerce-appointments' ), 'manage_woocommerce', 'addons', array( $this, 'global_addons_admin' ) );
	...
	public function script_styles() {
		...
		wp_enqueue_media();
		wp_enqueue_style( 'woocommerce_product_addons_css', WC_PRODUCT_ADDONS_PLUGIN_URL . '/assets/css/admin.css', [], WC_PRODUCT_ADDONS_VERSION );
		...
		$params = array(
			...
			'i18n'     => array(
				'required_fields'        => __( 'All fields must have a title and/or option name. Please review the settings highlighted in red border.', 'woocommerce-appointments' ),
				'limit_price_range'      => __( 'Limit price range', 'woocommerce-appointments' ),
				'limit_quantity_range'   => __( 'Limit quantity range', 'woocommerce-appointments' ),
				...
				'confirm_remove_addon'   => __( 'Are you sure you want remove this add-on field?', 'woocommerce-appointments' ),
				'confirm_remove_option'  => __( 'Are you sure you want delete this option?', 'woocommerce-appointments' ),
	...
	public function global_addons_admin() {
		if ( ! empty( $_GET['add'] ) || ! empty( $_GET['edit'] ) ) {
			...
			if ( ! empty( $_GET['edit'] ) ) {
				...
				if ( ! $global_addon ) {
					echo '<div class="error">' . esc_html__( 'Error: Add-on not found', 'woocommerce-appointments' ) . '</div>';
					return;
				}
	...
	public function sanitize_addon( $addon ) {
		$sanitized = array(
			...
			'adjust_duration'                    => ! empty( $addon['adjust_duration'] ) ? 1 : 0,
			'duration_type'                      => sanitize_text_field( $addon['duration_type'] ),
			'duration'                           => floatval( sanitize_text_field( $addon['duration'] ) ),
			'wc_appointment_hide_duration_label' => ! empty( $addon['wc_appointment_hide_duration_label'] ) ? 1 : 0,
			'wc_appointment_hide_price_label'    => ! empty( $addon['wc_appointment_hide_price_label'] ) ? 1 : 0,
			'wc_appointment_show_on_top'         => ! empty( $addon['wc_appointment_show_on_top'] ) ? 1 : 0,
		);
		...
		if ( is_array( $addon['options'] ) ) {
			...
			foreach ( $addon['options'] as $key => $option ) {
				$sanitized['options'][ $key ] = array(
					...
					'duration_type' => sanitize_text_field( $option['duration_type'] ),
					'duration'      => floatval( sanitize_text_field( $option['duration'] ) ),
				);
			}
		}
