...
class WC_Product_Addons_Display {
	...
	public function styles() {
		...
		if (
			...
			wp_enqueue_style( 'woocommerce-addons-css', WC_PRODUCT_ADDONS_PLUGIN_URL . '/assets/css/frontend.css', null, WC_PRODUCT_ADDONS_VERSION );
	...
	public function display( $post_id = false, $prefix = false ) {
		...
		if ( is_array( $product_addons ) && count( $product_addons ) > 0 ) {
			...
			foreach ( $product_addons as $addon ) {
				...
				echo $this->get_addon_html( $addon, $product_addons );
	 ...
	 * @return string
	 */
	public function get_addon_html( $addon, $product_addons ) {
		ob_start();

		$method_name = 'get_' . $addon['type'] . '_html';

		if ( is_callable( array( $this, $method_name ) ) ) {
			$this->$method_name( $addon, $product_addons );
		}

		do_action( 'woocommerce_product_addons_get_' . $addon['type'] . '_html', $addon, $product_addons );
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_multiple_choice_html( $addon, $product_addons ) {
		switch ( $addon['display'] ) {
			case 'images':
				$this->get_image_html( $addon, $product_addons );
				break;
			case 'radiobutton':
				$this->get_radiobutton_html( $addon, $product_addons );
				break;
			case 'select':
				$this->get_select_html( $addon, $product_addons );
				break;
		}
	}
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_image_html( $addon, $product_addons ) {
		wc_get_template(
			'addons/image.php',
			array(
				'addons' => $product_addons,
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_checkbox_html( $addon, $product_addons ) {
		wc_get_template(
			'addons/checkbox.php',
			array(
				'addons' => $product_addons,
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_radiobutton_html( $addon, $product_addons ) {
		wc_get_template(
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_select_html( $addon, $product_addons ) {
		wc_get_template(
			'addons/select.php',
			array(
				'addons' => $product_addons,
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_text_html( $addon, $product_addons ) {
		wc_get_template(
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_textarea_html( $addon, $product_addons ) {
		wc_get_template(
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_file_upload_html( $addon, $product_addons ) {
		wc_get_template(
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_price_html( $addon, $product_addons ) {
		wc_get_template(
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_input_multiplier_html( $addon, $product_addons ) {
		wc_get_template(
