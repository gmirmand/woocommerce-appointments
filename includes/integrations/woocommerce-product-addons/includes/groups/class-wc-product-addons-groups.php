...
class WC_Product_Addons_Groups {
	...
	public static function get_group( $id ) {
		...
		return new WP_Error( 'invalid_id', esc_html__( 'Unable to retrieve data. Invalid global add ons group or product ID.', 'woocommerce-appointments' ) );
	}
	...
	public static function delete_group( $id ) {
		if ( ! self::is_a_global_group_id( $id ) ) {
			return new WP_Error( 'invalid_id', esc_html__( 'Unable to delete group. Invalid global add ons group ID.', 'woocommerce-appointments' ) );
		}
