...
<div class="wc-pao-addon-container <?php echo $required ? 'wc-pao-required-addon' : ''; ?> wc-pao-addon wc-pao-addon-<?php echo esc_attr( sanitize_title( $name ) ); ?>" data-product-name="<?php esc_attr_e( $product_title ); ?>">
		...
		<?php
			...
			switch ( $title_format ) {
				case 'heading':
					...
					><?php echo wp_kses_post( wptexturize( $name ) ); ?> <?php echo $required ? '<em class="required" title="' . esc_attr__( 'Required field', 'woocommerce-appointments' ) . '">*</em>&nbsp;' : ''; ?><?php echo wp_kses_post( $price_display . $duration_display ); ?>
					</h2>
					<?php
					break;
				case 'hide':
